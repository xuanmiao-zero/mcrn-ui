...
import { SEARCH_ICON_BLACK, SEARCH_ICON_DEL } from 'MCImage';
...
class Search extends PureComponent {
  constructor(props) {
    ...
    this.keyword = props.defaultValue;
    /**
     * Animated values
     */
  ...
  componentDidMount() {
    // this.focus = this.refs.input_keyword._component.focus;
    if (this.autoFocus) {
  ...
  onSearch = async () => {
    this.props.beforeSearch &&
      (await this.props.beforeSearch(this.keyword));
    if (this.props.keyboardShouldPersist === false) {
      await Keyboard.dismiss();
    }
    this.props.onSearch && (await this.props.onSearch(this.keyword));
    this.props.afterSearch &&
      (await this.props.afterSearch(this.keyword));
  };
  ...
  onChangeText = async (text) => {
    ...
    this.props.onChangeText &&
      (await this.props.onChangeText(this.keyword));
  };
  ...
  onFocus = async () => {
    ...
    this.props.onFocus && (await this.props.onFocus(this.keyword));
  ...
  expandAnimation = () => new Promise((resolve) => {
    Animated.parallel([
      ...
      Animated.timing(this.iconDeleteAnimated, {
        toValue: this.keyword.length > 0 ? 1 : 0,
        duration: 200,
      }).start(),
